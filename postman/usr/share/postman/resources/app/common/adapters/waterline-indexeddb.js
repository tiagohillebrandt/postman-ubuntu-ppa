...
module.exports = (function adapterIdb () {
  ...
  var adapter = {
    ...
    find: function find (datastoreName, query, cb) {
      ...
      if (_.get(query, 'criteria.where.and') || _.get(query, 'criteria.where.or')) {
        // TODO: Throw waterline compat error
        console.error(
          'Ignored: AND/OR filter is not supported',
          query.criteria.where
        );
      }

      var dexieCollection = db[query.using];

      // constructing where clause
      // TODO: this is not needed, as this needs to be done which AND or OR sup
      _.forEach(query.criteria.where, function (value, key) {
        // TODO: normalize criteria for all query types
        if (!value) {
          return;
        }

        if (value.in) {
          dexieCollection = dexieCollection.where(key).anyOf(value.in);
        }
        else {
          dexieCollection = dexieCollection.where(key).equals(value);
        }
      });

      dexieCollection
        .toArray()
        .then(function (records) { cb(null, records); })
        .catch(function (e) { cb(e); });
    },
    ...
    destroy: function destroy (datastoreName, query, cb) {
      ...
      // we find the records first so we can bubble up the
      adapter.find(datastoreName, query, function (err, records) {
        if (err) {
          return cb(err);
        }

        deletePromises = _.map(records, function (record) {
          return db[query.using].delete(record[primaryKey]);
        });
