...
module.exports = (function waterlineNeDB () {
  ...
  var adapter = {
    ...
    identity: 'waterline-shared-nedb',
    ...
    registerDatastore: function registerDatastore (datastoreConfig, models, cb) {
      ...
      var datastore = {
        ...
        // We track all pending callbacks for requests, and call them when we get responses for those
        pendingCbs: new Map()
      };
      ...
      async.eachSeries(_.keys(models), function (modelIdentity, next) {
        ...
        var nedbConfig;
        if (datastore.config.dbPath) {
          var filename = path.join(datastore.config.dbPath, modelIdentity);
          nedbConfig = { filename: filename, inMemoryOnly: false };
        }
        else {
    ...
    define: function define (datastoreName, tableName, definition, cb) {
      ...
      // delegate to master if this adapter is a slave
      if (!datastore.config.master) {
        return delegateToMaster(datastore, 'define', [datastoreName, tableName, definition], cb);
      }
    ...
    setSequence: function setSequence (datastoreName, sequenceName, sequenceValue, cb) {
      ...
      // delegate to master if this adapter is a slave
      if (!datastore.config.master) {
        return delegateToMaster(datastore, 'setSequence', [datastoreName, sequenceName, sequenceValue], cb);
      }
